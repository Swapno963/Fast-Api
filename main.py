...
BAND = [
    ...
    {"id": 6, "name": "Band B", "genre": "Pop"},
    {
        ...
        "name": "Band B",
        "genre": "Pop",
        "albutms": [{"title": "Album 1", "release_date": "2020-03-24"}],
    },
...
async def index(
    ...
    if genre:
        filtered_bands = [
            b for b in bands_list if b.genre.value.lower() == genre.value.lower()
        ]
...
async def get_bands_by_genre(genre: GenreUrlChoices) -> list[dict]:
    ...
    print(
        f"Genra : {genre},           Filtered bands: {filtered_bands}"
    )  # Debugging line
    if not filtered_bands:
        raise HTTPException(status_code=404, detail="No bands found for this genre")
